...
var screenCatalog = []ScreenInfo{
	{ScreenOverview, "Overview", "", "See what differs, what needs attention, and where to go next.", "Overview is your starting point. It summarizes differences between this machine and your Blueprint profile, warnings, blocked work, and profile sync state. Open an item to jump to the screen where you can review or act on it.", "home summary attention warning difference drift"},
	{ScreenCapture, "Capture", SectionWorkflow, "Update your Blueprint profile from the system you're using now.", "Capture reads the current system and saves selected categories into your profile. Use it after you intentionally change your setup and want Blueprint to remember those changes. Review the categories, choose what to update, then capture.", "save remember update profile"},
	{ScreenRestore, "Restore", SectionWorkflow, "Preview and apply what your Blueprint profile would recreate on this machine.", "Restore compares your profile with this machine and shows what would change before anything is applied. Normal mode avoids supported conflicts; Forced mode can resolve some conflicts in favour of the profile. Review the consequences before applying a plan.", "rebuild recreate apply recover"},
	{ScreenPackages, "Packages", SectionSoftware, "Packages and tools Blueprint expects to be installed.", "Packages shows captured system packages, AUR packages, and global Mise tools, plus differences on this machine. Included items are part of portable restore; excluded or machine-specific items are remembered without being restored everywhere.", "software apps aur pacman mise tools install installed"},
	...
	{ScreenMachines, "Machines", SectionProfile, "Per-machine locations for Resources that cannot use one shared path.", "Machines lets a portable Resource use a different path on a particular computer without changing its shared definition or contents. Select a machine to use its mappings, or leave the portable defaults active when no override is needed.", "computer laptop desktop host paths mappings"},

...
2. **Capture navigation and actions:** Confirm the sidebar begins Overview → Capture → Packages. In Capture, use `Space` to toggle one category, `a` to select changed categories, `c` to review the selection, and `C` to review every category. Confirm the review groups Changes, Preserved by policy, and Blocked, names the policy scope, and that `Space`/`x` on a target recalculate it while Blocked targets refuse changes. Confirm `Enter` shows an approval summary, approving captures, `Esc` cancels without mutation, and a successful capture refreshes Overview, category status, and local Profile Git/Sync state without committing or pushing.

...
2. **Capture navigation and actions:** Confirm the sidebar begins Overview → Capture → Packages. In Capture, use `Space` to toggle one provider, `a` to select changed providers, `c` to capture the selection, and `C` for aggregate Capture All. Confirm `Enter` approves the modal, `Esc` cancels without mutation, and a successful capture refreshes Overview, provider status, and local Profile Git/Sync state without committing or pushing.

...
# Usage: soak.sh <reboot-cycles|fresh-overlay> <count> [installer|tty0-console|ttys0-console|rebuilt|debug]
...
trap 'ra_finish "$?"' EXIT
experiment=${1:?usage: soak.sh <reboot-cycles|fresh-overlay> <count> [installer|tty0-console|ttys0-console|rebuilt|debug]}
count=${2:?usage: soak.sh <reboot-cycles|fresh-overlay> <count> [installer|tty0-console|ttys0-console|rebuilt|debug]}
boot_image=${3:-installer}
[[ $boot_image =~ ^(installer|tty0-console|ttys0-console|rebuilt|debug)$ ]] || { ra_note "unknown boot image: $boot_image"; exit 2; }
...
ra_install_base "$RA_WORK" "$boot_image"
ra_pass OMARCHY_INSTALL

if [[ $experiment == fresh-overlay ]]; then
  ra_fresh_overlay_trials "$count" "$boot_image"
else
